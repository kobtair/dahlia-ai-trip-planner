                ...
                <TabsContent value="plan" className="pt-6">
                  <div className="grid gap-7 xl:grid-cols-[minmax(0,1fr)_260px]">
                    ...
                    <aside className="space-y-3">
                      ...
                      {plan.weather.available ? <div className="rounded-2xl border border-border p-5"><h3 className="flex items-center gap-2 text-sm font-semibold"><CloudSun className="size-4" /> Live forecast</h3><div className="mt-4 space-y-2">{plan.weather.daily?.slice(0, 5).map((day) => <div key={day.date} className="flex items-center justify-between text-xs"><span className="text-muted-foreground">{formatDate(day.date)}</span><span>{day.high}° / {day.low}° · {day.precipitation}% rain</span></div>)}</div><p className="mt-4 text-[10px] text-muted-foreground">Open-Meteo · fetched {new Date(plan.meta.fetchedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p></div> : <div className="rounded-2xl border border-border p-5"><h3 className="text-sm font-semibold">Forecast timing</h3><p className="mt-2 text-xs leading-5 text-muted-foreground">{plan.weather.note}</p></div>}
                    </aside>
                  </div>
